...
export class IpRateGuard extends BaseRateLimitGuard {
    async canActivate(context: ExecutionContext): Promise<boolean> {
        const customEndpointLimits = this.reflector.get<RateLimitConfig>('endpointRateLimits', context.getHandler());
        ...
        if(customEndpointLimits){
            ...
            console.debug('Using custom endpoint limit', { minLimit, hourLimit, secLimit });
        }
